...
public class StudentCheckInResourceIt {
    private static final String STUDENT_CHECK_IN_URL = "http://localhost:8080/api/student/lesson/in";
    ...
    @Autowired
    public StudentCheckInResourceIt(TestRestTemplate testRestTemplate) {
    ...
    @Test
    public void studentShouldJoinToSectionOnTime() {
        ...
        String shouldBe = LessonStatus.JOIN_TO_SESSION_NOT_ON_TIME.name();
        Assertions.assertEquals(code, shouldBe);
    }

    @Test
    public void studentShouldJoinToSectionOnCorrectTime() {
        String sectionName = "C0002.02";
        RequestBody3DTO requestBody3DTO = new RequestBody3DTO();
        requestBody3DTO.setSectionName(sectionName);
        HttpEntity<RequestBody3DTO> httpEntity = new HttpEntity<>(requestBody3DTO, headers);
        ...
        Map body = exchange.getBody();
        Object code = body.get("code");
        String shouldBe = LessonStatus.JOIN_SESSION_IS_ACCEPTED.name();
        Assertions.assertEquals(code, shouldBe);
    }
