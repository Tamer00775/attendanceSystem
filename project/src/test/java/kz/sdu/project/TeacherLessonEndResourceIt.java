...
public class TeacherLessonEndResourceIt {
    private static final String LOGIN_URL = "http://localhost:8081/open-api/auth/login";
    private static final String TEACHER_START_LESSON = "http://localhost:8081/api/teacher/end";

    private final TestRestTemplate testRestTemplate;
    private HttpHeaders headers;

    @Autowired
    public TeacherLessonEndResourceIt(TestRestTemplate testRestTemplate) {
    ...
    @Test
    public void sectionCodeMustBeCorrectAndStartOnTime1() {
        String sectionName= "C0001.02";
        RequestBody3DTO requestBody3DTO = new RequestBody3DTO();
        requestBody3DTO.setSectionName(sectionName);
        HttpEntity<RequestBody3DTO> httpEntity = new HttpEntity<>(requestBody3DTO, headers);
        ResponseEntity<Map> exchange = testRestTemplate.exchange(TEACHER_START_LESSON, HttpMethod.POST, httpEntity, Map.class);
        Object code = exchange.getBody().get("code");
        String shouldBe = LessonStatus.START_LESSON_PROCESS_IS_FAILED.name();
        Assertions.assertEquals(code, shouldBe);
    }

    @Test
    public void sectionCodeMustBeCorrectAndStartOnTime2() {
        ...
        ResponseEntity<Map> exchange = testRestTemplate.exchange(TEACHER_START_LESSON, HttpMethod.POST, httpEntity, Map.class);
        Object code = exchange.getBody().get("code");
        String shouldBe = LessonStatus.START_LESSON_PROCESS_IS_SUCCESSFULLY.name();
        Assertions.assertEquals(code, shouldBe);
    }
